...
impl OpenLiteSpeedService {
    ...
    pub fn generate_vhost_config(
        ...
        if let (Some(cert), Some(key)) = (ssl_cert_path, ssl_key_path) {
            ...
            // A path like /etc/letsencrypt/../../../etc/shadow would pass the
            // starts_with check without this guard.
            if cert.contains("..") || key.contains("..") {
                return Err(ServiceError::CommandFailed(
                    "SSL cert/key paths must not contain '..' sequences".to_string(),
                ));
            }
...
impl OpenLiteSpeedService {
    ...
    pub async fn configure_panel_ssl(
        ...
        // Validate inputs before writing to the OLS config file to prevent
        // injection of extra OLS directives (OLS-03: config injection).
