...
impl UfwService {
    ...
    pub async fn import_rules(&self, content: &str) -> Result<(), ServiceError> {
        ...
        if content.contains(":INPUT ACCEPT") || content.contains(":FORWARD ACCEPT") {
            return Err(ServiceError::CommandFailed(
                "Rejected: rules file contains an open default policy (':INPUT ACCEPT' or ':FORWARD ACCEPT')".to_string(),
            ));
        }
        ...
        let tmp_path = "/etc/ufw/user.rules.panel_tmp";
        fs::write(tmp_path, content)
            .await
            .map_err(|e| ServiceError::IoError(e.to_string()))?;
        fs::rename(tmp_path, "/etc/ufw/user.rules")
            .await
            .map_err(|e| ServiceError::IoError(e.to_string()))?;
        self.reload().await?;
