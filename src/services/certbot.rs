...
impl CertbotService {
    ...
    pub async fn issue_certificate(
        ...
        if !output.status.success() {
            ...
            tracing::error!(domain = %domain, "Certbot failed: {}", stderr);
            return Err(ServiceError::CommandFailed(
                "Certificate issuance failed. Check server logs for details.".to_string(),
            ));
        }
    ...
    pub async fn issue_certificate_with_www(
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            tracing::error!(domain = %domain, "Certbot (with www) failed: {}", stderr);
            return Err(ServiceError::CommandFailed(
                "Certificate issuance failed. Check server logs for details.".to_string(),
            ));
        }
    ...
    pub async fn revoke_certificate(&self, domain: &str) -> Result<(), ServiceError> {
        ...
        if !output.status.success() {
            // SEC-35-06: log stderr server-side only; never expose certbot
            // internals (paths, ACME details) to the API response.
            let stderr = String::from_utf8_lossy(&output.stderr);
            tracing::error!(domain = %domain, "certbot revoke failed: {}", stderr);
            return Err(ServiceError::CommandFailed(
                "Certificate revocation failed. Check server logs for details.".to_string(),
            ));
        }
    ...
    pub async fn renew_all(&self) -> Result<(), ServiceError> {
        ...
        if !output.status.success() {
            // SEC-35-06: log stderr server-side; return opaque message to caller.
            let stderr = String::from_utf8_lossy(&output.stderr);
            tracing::error!("certbot renew_all failed: {}", stderr);
            return Err(ServiceError::CommandFailed(
                "Certificate renewal failed. Check server logs for details.".to_string(),
            ));
        }
    ...
    pub async fn has_certificate(&self, domain: &str) -> bool {
        // Reject invalid domain names to prevent path traversal via CERTBOT_LIVE_DIR.
        if crate::utils::validators::validate_domain(domain).is_err() {
            return false;
        }
        let cert_path = format!("{}/{}/fullchain.pem", CERTBOT_LIVE_DIR, domain);
