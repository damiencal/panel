...
impl SshHardeningService {
    ...
    pub async fn apply_config(
        ...
        for user in &config.allowed_users {
            // SEC-35-07: reject whitespace so callers cannot inject multiple
            // users in a single entry (sshd_config splits AllowUsers on spaces).
            if user.chars().any(|c| c.is_whitespace()) {
                return Err(ServiceError::CommandFailed(
                    "AllowUsers entry must not contain whitespace — provide one username per entry"
                        .to_string(),
                ));
            }
        }
        // SEC-35-02: enforce an allowlist for PermitRootLogin and a safe-char
        // check for crypto algorithm string fields.
        const PERMIT_ROOT_VALID: &[&str] =
            &["yes", "no", "prohibit-password", "forced-commands-only"];
        if !PERMIT_ROOT_VALID.contains(&config.permit_root_login.as_str()) {
            return Err(ServiceError::CommandFailed(format!(
                "PermitRootLogin must be one of: {}",
                PERMIT_ROOT_VALID.join(", ")
            )));
        }
        ...
        for (field_name, field_val) in &[("MaxStartups", config.max_startups.as_str())] {
        ...
        // MaxStartups must match the sshd_config format: "start" or "start:rate:full"
        {
            let ms = config.max_startups.trim();
            let parts: Vec<&str> = ms.split(':').collect();
            let valid = match parts.len() {
                1 => parts[0].parse::<u32>().is_ok(),
                3 => parts.iter().all(|p| p.parse::<u32>().is_ok()),
                _ => false,
            };
            if !valid {
                return Err(ServiceError::CommandFailed(
                    "MaxStartups must be a number or 'start:rate:full' (e.g. '10:30:100')"
                        .to_string(),
                ));
            }
        }
