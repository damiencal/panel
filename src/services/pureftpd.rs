...
impl PureFtpdService {
    ...
    pub async fn configure_virtual_hosting(&self) -> Result<(), ServiceError> {
        ...
        // TLS — read operator configuration.
        // 0=disable, 1=optional (legacy clients may use plaintext), 2=required
        // Default to 1 (optional) if the config cannot be loaded, so that
        ...
        self.set_config("TLS", tls_mode).await?;
    ...
    pub async fn create_user(
        ...
        crate::utils::validators::validate_safe_path(home_dir, "/home/")
            .map_err(|e| ServiceError::CommandFailed(e.to_string()))?;
        // SEC-34-06: validate_safe_path only checks the prefix; also ensure no colon
        // characters that would corrupt the colon-delimited passwd entry.
        crate::utils::validators::validate_passwd_field(home_dir, "home_dir")
            .map_err(ServiceError::CommandFailed)?;
        ...
        // SEC-34-07: atomic write — write to a .tmp file then rename so a mid-write
        // kill cannot leave the passwd file partially written and corrupt the PDB.
    ...
    pub async fn delete_user(&self, username: &str) -> Result<(), ServiceError> {
        ...
        // SEC-34-07: atomic write to prevent partial-write corruption.
        let tmp_path = format!("{}.tmp", PUREFTPD_PASSWD_FILE);
    ...
    pub async fn update_home_dir(
        ...
        let new_content: String = content
            ...
            .map(|line| {
                if line.starts_with(&format!("{}:", username)) {
                    // FTP-02: Pure-FTPd passwd format is 7 colon-separated fields:
                    //   login:crypted_pass:uid:gid:gecos:home_dir:shell
                    // splitn(6) merges home_dir and shell into parts[5], causing
                    // new_home_dir to land in the gecos slot.  Use splitn(7)
                    // to address each field individually.
    ...
    async fn rebuild_pdb(&self) -> Result<(), ServiceError> {
        // FTP-01: actually regenerate the binary PDB from the passwd file.
        // Without this, deleted/changed accounts remain valid in the stale PDB
        // when Pure-FTPd is configured with `PureDB`.
        shell::exec(
            "pure-pw",
            &["mkdb", PUREFTPD_PDB_FILE, "-f", PUREFTPD_PASSWD_FILE],
        )
        .await
        .map_err(|e| ServiceError::CommandFailed(format!("pure-pw mkdb: {}", e)))?;
        tracing::info!("Pure-FTPd PDB rebuilt from passwd file");
        Ok(())
    }
