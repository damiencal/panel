...
/// Validate a Cloudflare object ID — must be exactly 32 lowercase hex characters.
fn validate_cf_id(id: &str, label: &str) -> Result<(), CloudflareError> {
    if id.len() != 32 || !id.chars().all(|c| matches!(c, '0'..='9' | 'a'..='f')) {
        return Err(CloudflareError::Api(format!(
            "Invalid {label} ID format (expected 32-char hex)"
        )));
    }
    Ok(())
}
...
impl CloudflareClient {
    ...
    pub async fn get_zone(&self, cf_zone_id: &str) -> Result<CfZone, CloudflareError> {
        validate_cf_id(cf_zone_id, "zone")?;
        let resp: CfResponse<CfZone> = self
            .http
    ...
    pub async fn delete_record(
        ...
        validate_cf_id(cf_zone_id, "zone")?;
        validate_cf_id(cf_record_id, "record")?;
        let resp: CfResponse<serde_json::Value> = self
            .http
            .delete(format!(
