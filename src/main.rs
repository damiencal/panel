...
impl RequestAccessLayer {
    ...
    fn enforce_rate_limit(&self, client_ip: &str) -> Result<(), u64> {
        ...
        // RATE-MEM-01: cap the number of tracked IPs to prevent memory exhaustion
        // from an attacker rotating through a large number of source IPs.
        ...
        // If we are still at the cap after expiry-based eviction, remove the entry
        // whose most-recent request is the oldest (least-active IP).
...
fn is_protected_request_path(path: &str) -> bool {
    // The phpMyAdmin proxy must also go through the IP-allowlist and rate-limiter
    // middleware — without this it would be reachable without any panel-level
    // access control.
    path.starts_with("/_dioxus/") || path.starts_with("/api/") || path.starts_with("/phpmyadmin/")
}
...
async fn request_access_middleware(
    ...
    let client_ip = if state.trust_proxy_headers {
        panel::auth::guards::extract_client_ip_from_headers(req.headers())
            .unwrap_or_else(|| "unknown".to_string())
    } else {
        // When trust_proxy_headers is false, X-Forwarded-For is untrusted.
        // Use the direct connection address to prevent IP-allowlist bypass and
        // rate-limit evasion via spoofed proxy headers.
        req.extensions()
...
async fn phpmyadmin_proxy_handler(
    ...
    // Require a valid panel session before proxying any phpMyAdmin traffic.
    // The path is now included in is_protected_request_path() so the IP-allowlist
    // and coarse rate limiter run first; this inner check adds the JWT gate.
    ...
    let jwt_token = match parse_cookie_value_str(&cookie_header, "auth_token") {
        Some(t) => t.to_string(),
        None => return (http::StatusCode::UNAUTHORIZED, "Authentication required").into_response(),
    };
    if panel::auth::jwt::verify_token(&jwt_token).is_err() {
