...
pub const SUPPORTED_PHP_VERSIONS: &[&str] = &["7.4", "8.3", "8.4"];
...
pub struct Site {
    ...
    /// Stored path is internal; never returned to clients (used for server-side TLS provisioning only).
    #[serde(skip_serializing)]
    pub ssl_certificate: Option<String>,
    /// Stored path is internal; never returned to clients.
    #[serde(skip_serializing)]
...
pub struct BasicAuthUser {
    ...
    /// HASH-01: Never serialise the hash to clients; it serves no UI purpose
    /// and would expose the htpasswd credential material.
    #[serde(skip_serializing)]
