...
pub(crate) mod helpers {
    ...
    pub async fn ensure_init() -> Result<(), String> {
        SERVER_INIT
            .get_or_try_init(|| async {
                ...
                crate::auth::jwt::init_jwt_key(config.server.secret_key.clone());
                // Derive and cache the AES-256-GCM key for TOTP secret encryption
                // from the same master secret, domain-separated by HMAC label.
                crate::auth::init_totp_enc_key(&config.server.secret_key);
                crate::db::init_pool(&config.database.url)
                    .await
                    .map_err(|e| e.to_string())?;

                // Propagate proxy-header trust flag to the login rate limiter.
                crate::server::auth::init_proxy_trust(config.request_security.trust_proxy_headers);
    ...
    /// Reject a JWT if the user changed their password after the token was issued.
    ///
    /// Call this after `verify_auth()` + `get_pool()` in any endpoint that is
    /// sensitive to SESS-01 (password change, 2FA management, admin actions).
    /// For most read-only endpoints the overhead isn't warranted; the 24-hour
    /// token window is the accepted trade-off there.
    #[cfg(feature = "server")]
    pub async fn check_token_not_revoked(
        pool: &sqlx::SqlitePool,
        claims: &crate::models::auth::JwtClaims,
    ) -> Result<(), ServerFnError> {
        let user = crate::db::users::get(pool, claims.sub)
            .await
            .map_err(|_| ServerFnError::new("Session invalidated"))?;
        // SUSP-COMBO-01 Part A: reject all requests from suspended accounts.
        if user.status == crate::models::user::AccountStatus::Suspended {
            return Err(ServerFnError::new(
                "Account suspended. Please contact support.",
            ));
        }
        if let Some(changed_at) = user.password_changed_at {
            // claims.iat is seconds since UNIX epoch; compare to changed_at.
            if claims.iat < changed_at.timestamp() {
                return Err(ServerFnError::new(
                    "Session invalidated due to recent password change. Please log in again.",
                ));
            }
        }
        Ok(())
