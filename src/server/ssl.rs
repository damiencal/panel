...
pub async fn server_get_ssl_status() -> Result<SslStatus, ServerFnError> {
    ...
    let panel_cert_path = format!("/etc/letsencrypt/live/{}/fullchain.pem", panel_hostname);
    let mail_cert_path = format!("/etc/letsencrypt/live/{}/fullchain.pem", mail_hostname);
