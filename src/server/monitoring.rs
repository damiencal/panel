...
pub async fn server_get_audit_log(limit: i64) -> Result<Vec<AuditLogEntry>, ServerFnError> {
    ...
    let rows = sqlx::query_as::<_, AuditLogEntryRow>(
        "SELECT id, user_id, action, target_type, target_id, target_name, \
         description, status, error_message, ip_address, created_at \
...
pub async fn server_kill_process(pid: u32, force: bool) -> Result<(), ServerFnError> {
    ...
    if !output.status.success() {
        ...
        return Err(ServerFnError::new(format!("kill failed: {}", err)));
    }
...
pub async fn server_trigger_os_update() -> Result<OsUpdateResult, ServerFnError> {
    ...
    // Step 1: clear any stale apt lock files (no-op if a real process holds them)
    for lock in &[
        ...
        // fuser exits 0 when a process holds the file, non-zero when no one does.
        // Only remove when no live process owns the lock.
        let held = std::process::Command::new("fuser")
            .arg(lock)
    ...
    // Reconfigure any partially-configured packages before updating
...
pub async fn server_trigger_panel_update() -> Result<PanelUpdateResult, ServerFnError> {
    ...
    Command::new("chmod")
        ...
        .status()
        .await
        .map_err(|e| ServerFnError::new(format!("chmod failed: {e}")))?;

    Command::new("chown")
        .args(["panel:panel", tmp_binary])
        .status()
        .await
        .map_err(|e| ServerFnError::new(format!("chown failed: {e}")))?;
