...
pub async fn server_save_branding(input: BrandingInput) -> Result<i64, ServerFnError> {
    ...
    // Validate accent_color is a valid CSS hex colour (#RRGGBB).
    // Check length, leading '#', and that all subsequent chars are hex digits.
    let color = &input.accent_color;
    if color.len() != 7
        || !color.starts_with('#')
        || !color[1..].chars().all(|c| c.is_ascii_hexdigit())
    {
        return Err(ServerFnError::new(
            "accent_color must be a valid 6-digit hex color (e.g. #F43F5E)",
        ));
    }
