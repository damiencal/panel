...
pub async fn server_save_spam_filter_settings(
    ...
    // Validate the quarantine address to prevent an invalid/malicious email
    // address from being written to the mail-filter configuration.
    if let Some(ref qmb) = quarantine_mailbox {
        crate::utils::validators::validate_email(qmb)
            .map_err(|_| ServerFnError::new("Invalid quarantine mailbox address"))?;
    }
...
pub async fn server_get_mail_logs(
    ...
    // Cap search string to prevent ReDoS / excessive memory allocation in the
    // per-line contains() scan below.
    let search = search.map(|s| {
...
async fn read_mail_logs(limit: usize, search: Option<&str>) -> Result<Vec<EmailLogEntry>, String> {
    ...
    // Try common mail log paths — /var/log/syslog is intentionally excluded
    // because it contains non-mail OS events; exposing it via the mail-log API
    // would leak unrelated sensitive information.
    let log_paths = ["/var/log/mail.log", "/var/log/maillog"];
...
async fn debug_domain(domain: &str) -> Result<EmailDebugResult, String> {
    ...
    if let Some(mx) = mx_records.first() {
        ...
        // SSRF prevention: resolve the MX hostname and reject any result that
        // resolves to a private, loopback, or link-local address.  An attacker
        // who controls a DNS MX record could otherwise point it at an internal
        // service (e.g. 169.254.0.1, 10.0.0.1) and use this probe to perform
        // server-side port scanning.
        let ok_to_probe = {
            use std::net::IpAddr;
            match tokio::net::lookup_host(format!("{host}:25")).await {
                Ok(addrs) => {
                    let mut safe = false;
                    for addr in addrs {
                        let ip = addr.ip();
                        ...
                        if private {
                            notes.push(format!(
                                "MX host {host} resolves to a private/internal address — probe blocked"
                            ));
                            safe = false;
                            break;
                        }
                ...
                Err(_) => {
                    notes.push(format!("MX host {host} could not be resolved"));
                    false
                }
            }
        };
