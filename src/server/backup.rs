...
fn validate_destination(dest: &str) -> Result<(), &'static str> {
    ...
    // Absolute paths must be within /home/ to prevent users from scheduling
    // backups that write to sensitive system directories (BACKUP-05).
    if dest.starts_with('/') && !dest.starts_with("/home/") {
        return Err("Absolute backup destinations must be within /home/");
    }
    // SEC-33-05: reject tilde-paths that target other users' home dirs
    // (e.g. "~root/backups") — they bypass the absolute-path check above and
    // would be expanded by shellexpand::tilde() to "/root/backups" at execution,
    // triggering the validate_safe_path guard but only after leaking the
    // expanded path in the error message.
    if dest.starts_with('~') && dest.len() > 1 && !dest.starts_with("~/") {
        return Err(
            "Backup destination cannot target other users' home directories. Use ~/path instead.",
        );
    }
    Ok(())
}
...
pub async fn server_create_backup_schedule(
    ...
    const ALLOWED_STORAGE_TYPES: &[&str] = &["local", "s3", "sftp"];
    if !ALLOWED_STORAGE_TYPES.contains(&req.storage_type.as_str()) {
        return Err(ServerFnError::new("Invalid storage type"));
    }
...
async fn backup_site(
    ...
    // After tilde expansion, ensure the resolved path is still inside /home/
    // to prevent writing archives to arbitrary system locations (e.g. ~root → /root/).
    crate::utils::validators::validate_safe_path(&dest_dir, "/home/")
        .map_err(|_| "Backup destination must be within /home/".to_string())?;
...
async fn backup_mailbox(
    ...
    // Guard against path-traversal components that could escape the mail spool root.
    // Both values come from the DB (trusted insert-time validation), but defence-in-depth
    // requires we confirm neither contains ".." before embedding in a filesystem path.
    if local_part.contains("..")
        || domain.contains("..")
        || local_part.contains('/')
        || domain.contains('/')
        || local_part.contains('\0')
        || domain.contains('\0')
    {
        return Err(format!(
            "Refusing to back up mailbox: unsafe characters in local_part={local_part:?} or domain={domain:?}"
        ));
    }
    // Standard Dovecot maildir layout: /var/mail/vhosts/<domain>/<local_part>
    let maildir = format!("/var/mail/vhosts/{domain}/{local_part}");
    // Canonicalize to catch any remaining traversal via the resolved path.
    let maildir_canonical = std::fs::canonicalize("/var/mail/vhosts")
        .unwrap_or_else(|_| std::path::PathBuf::from("/var/mail/vhosts"));
    let maildir_path = std::path::Path::new(&maildir);
    if !maildir_path.starts_with(&maildir_canonical) {
        // The constructed path escapes the mail spool root — refuse.
        return Err(format!(
            "Refusing to back up mailbox: resolved path {maildir:?} is outside /var/mail/vhosts"
        ));
    }
