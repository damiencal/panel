...
pub async fn server_create_email_domain(domain: String) -> Result<i64, ServerFnError> {
    ...
    let id = crate::db::email::create_domain(pool, claims.sub, domain.clone())
        .await
        .map_err(|e| {
            // INFO-LEAK-01: log full error server-side; return only an opaque message
            // so SQLite schema details (table/column names) are not sent to the client.
            let msg = e.to_string();
            if msg.contains("UNIQUE") {
                ServerFnError::new("Domain already exists")
            } else {
                tracing::warn!("DB error in create_email_domain: {e}");
                ServerFnError::new("Failed to create email domain")
            }
        })?;
...
pub async fn server_create_mailbox(
    ...
    // '/' is explicitly excluded to prevent path traversal in /var/mail/vhosts/{domain}/{local_part} paths.
    // ".." is also blocked to prevent directory traversal even in contexts where '/' is absent.
    ...
    {
        return Err(ServerFnError::new(
            "Invalid mailbox local part: must not contain '/', null bytes, newlines, or '..'.",
        ));
    }
    {
...
pub async fn server_create_forwarder(
    ...
    // Validate local_part: same RFC 5321-safe character set as mailbox creation.
    // '/' is explicitly excluded to prevent path traversal in maildir paths.
    // ".." is also blocked for the same reason.
    if local_part.contains('/')
        || local_part.contains('\0')
        || local_part.contains('\n')
        || local_part.contains("..")
    {
        return Err(ServerFnError::new(
            "Invalid forwarder local part: must not contain '/', null bytes, newlines, or '..'.",
        ));
    }
    {
        let lp_re = regex::Regex::new(r"^[a-zA-Z0-9.!#$%&'*+=?^_`{|}~-]{1,64}$")
        ...
        if !lp_re.is_match(&local_part) {
            return Err(ServerFnError::new(
                "Invalid forwarder local part. Use only letters, digits, and standard email special characters (max 64 chars)."
            ));
        }
    }
...
pub async fn server_create_regex_forwarder(
    ...
    // Block characters that act as Postfix regexp-map delimiters or inject
    // extra lines into the map file, preventing pattern-injection attacks.
    if pattern.contains('/')
        || pattern.contains('\n')
        || pattern.contains('\r')
        || pattern.contains('\0')
    {
        return Err(ServerFnError::new(
            "Pattern must not contain '/', newlines, or null bytes",
        ));
    }
...
pub async fn server_create_mailbox_backup(
    ...
    // Use panel- prefix so the janitor's panel-* glob cleans these up automatically.
    let dest_path = format!("/tmp/panel-mailbackup-{}.tar.gz", token);
