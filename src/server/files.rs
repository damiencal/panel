...
pub async fn server_fm_set_permissions(
    ...
    // AUDIT-06: In a shared-hosting environment the world-write (0o002) bit
    // lets processes running as other hosted-site UIDs overwrite this file,
    // enabling cross-account backdoor injection.  Cap the mask to 0o775 so
    // group-write is still available but world-write is never set.
    if mode & 0o002 != 0 {
        return Err(ServerFnError::new(
            "World-writable permissions (e.g. 777, 666, 002) are not permitted on this server",
        ));
    }
...
pub async fn server_fm_extract_archive(
    ...
    let _abs_str = abs_path.to_string_lossy().to_string();
    let dest_str = dest_dir.to_string_lossy().to_string();

    if filename.ends_with(".zip") {
        // Copy the archive to a stable temp path before inspection so that a
        // concurrent file-upload cannot swap in a malicious archive between the
        // zip-slip guard (step 1) and the extraction (step 2) — TOCTOU fix.
        let tmp_zip = format!("/tmp/panel-extract-{}.zip", uuid::Uuid::new_v4());
        tokio::fs::copy(&abs_path, &tmp_zip)
            .await
            .map_err(|e| ServerFnError::new(format!("Cannot stage archive: {e}")))?;
        // SEC-33-07: restrict staging file to owner-only to prevent other local
        // users (e.g., PHP running as another site's UID) from reading the archive.
        {
            use std::os::unix::fs::PermissionsExt;
            let _ = std::fs::set_permissions(&tmp_zip, std::fs::Permissions::from_mode(0o600));
        }

        // Zip-slip guard: reject ".." traversal AND absolute paths like "/etc/passwd".
        let list_out = crate::services::shell::exec("unzip", &["-Z1", &tmp_zip])
            .await
            .map_err(|e| {
                let _ = std::fs::remove_file(&tmp_zip);
                ServerFnError::new(format!("Cannot inspect archive: {}", e))
            })?;
        let listing = String::from_utf8_lossy(&list_out.stdout);
        for entry in listing.lines() {
            let has_unsafe = std::path::Path::new(entry).components().any(|c| {
                matches!(
                    c,
        ...
        crate::services::shell::exec("unzip", &["-o", &tmp_zip, "-d", &dest_str])
            .await
            .map_err(|e| {
                let _ = std::fs::remove_file(&tmp_zip);
                ServerFnError::new(format!("Extraction failed: {}", e))
            })?;
        let _ = std::fs::remove_file(&tmp_zip);
    } else if filename.ends_with(".tar.gz") || filename.ends_with(".tgz") {
        // Snapshot the archive before inspection so a concurrent rename/upload cannot
        // swap in a malicious payload between the header scan and extraction. (TOCTOU fix)
        let tmp_tar = format!("/tmp/panel-extract-{}.tar.gz", uuid::Uuid::new_v4());
        tokio::fs::copy(&abs_path, &tmp_tar)
            .await
            .map_err(|e| ServerFnError::new(format!("Cannot stage archive: {e}")))?;
        // SEC-33-07: restrict staging file to owner-only.
        {
            use std::os::unix::fs::PermissionsExt;
            let _ = std::fs::set_permissions(&tmp_tar, std::fs::Permissions::from_mode(0o600));
        }
        let abs_tmp = tmp_tar.clone();
        // Zip-slip guard: list entries (verbose to expose symlink targets) and reject
        // any path component containing ".." or absolute paths, as well as any
        // symlink whose target contains ".." or is absolute (symlink escape).
        let list_out = crate::services::shell::exec("tar", &["-tvzf", &abs_tmp])
            .await
            .map_err(|e| {
        ...
        let listing = String::from_utf8_lossy(&list_out.stdout);
        for entry in listing.lines() {
            // Verbose format: "lrwxrwxrwx ... link -> target"
            // Check the symlink target after " -> " when present.
            let (path_part, symlink_target) = if let Some(_arrow) = entry.find(" -> ") {
                let parts: Vec<&str> = entry.splitn(2, " -> ").collect();
                // path_part is the last field before " -> " in verbose output
                let fields: Vec<&str> = parts[0].split_whitespace().collect();
                (fields.last().copied().unwrap_or(""), Some(parts[1]))
            } else {
                let fields: Vec<&str> = entry.split_whitespace().collect();
                (fields.last().copied().unwrap_or(""), None)
            };
            let path_unsafe = std::path::Path::new(path_part).components().any(|c| {
                matches!(
                    c,
                    std::path::Component::ParentDir | std::path::Component::RootDir
                )
            ...
            if let Some(target) = symlink_target {
                let target_unsafe =
                    target.starts_with('/') || target.contains("../") || target.starts_with("..");
                if target_unsafe {
                    let _ = std::fs::remove_file(&tmp_tar);
                    return Err(ServerFnError::new(
                        "Archive contains symlinks with unsafe targets and cannot be extracted",
                    ));
                }
            }
        }
        ...
        crate::services::shell::exec(
            "tar",
            &["-xzf", &abs_tmp, "-C", &dest_str, "--no-absolute-filenames"],
        )
        .await
        .map_err(|e| {
            let _ = std::fs::remove_file(&tmp_tar);
        ...
        // Snapshot the archive before inspection so a concurrent rename/upload cannot
        // swap in a malicious payload between the header scan and extraction. (TOCTOU fix)
        let tmp_tar = format!("/tmp/panel-extract-{}.tar", uuid::Uuid::new_v4());
        tokio::fs::copy(&abs_path, &tmp_tar)
            .await
            .map_err(|e| ServerFnError::new(format!("Cannot stage archive: {e}")))?;
        // SEC-33-07: restrict staging file to owner-only.
        ...
        // any path component containing ".." or absolute paths, as well as any
        // symlink whose target contains ".." or is absolute (symlink escape).
        let list_out = crate::services::shell::exec("tar", &["-tvf", &abs_tmp])
            .await
            .map_err(|e| {
                let _ = std::fs::remove_file(&tmp_tar);
                ServerFnError::new(format!("Cannot inspect archive: {}", e))
        ...
        crate::services::shell::exec(
            "tar",
            &["-xf", &abs_tmp, "-C", &dest_str, "--no-absolute-filenames"],
        )
        .await
        .map_err(|e| {
            let _ = std::fs::remove_file(&tmp_tar);
...
pub async fn server_fm_write_text_file(
    ...
    // SEC-35-01: create temp file with 0o600 (owner-only) so its contents are
    // never world-readable, even briefly, in a shared-hosting environment.
    ...
    // For existing files, restore the original permission bits so the rename
    // preserves them.  For new files the 0o600 mode stands.
    if abs_path.exists() {
