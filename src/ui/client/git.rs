...
/// "No repo attached" panel — owns the attach form state.
#[component]
fn GitNoRepo(site_id: i64, on_attached: EventHandler<()>) -> Element {
        ...
        div { class: "glass-card rounded-2xl p-10",
            div { class: "text-center mb-8",
                ...
                p { class: "text-[13px] text-gray-400",
                    "Attach a Git repository to deploy and manage your site's code."
                }
            }
            if let Some(ref err) = attach_error() {
                div { class: "max-w-lg mx-auto bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg mb-4 text-sm",
                    "{err}"
                }
            }
            form {
                onsubmit: move |ev: FormEvent| {
                    ...
                    spawn(async move {
                        match server_attach_git_repo(site_id, url, br).await {
                            Ok(()) => on_attached(()),
...
/// Repo-attached view — owns its own status/branches/log resources and all action state.
#[component]
fn GitRepoPanel(
    site_id: i64,
    repo: panel::models::git::SiteGitRepoPublic,
    repo_resource: Resource<Result<Option<panel::models::git::SiteGitRepoPublic>, ServerFnError>>,
) -> Element {
    let mut repo_resource = repo_resource;
    let mut status_resource = use_resource(move || async move { server_git_status(site_id).await });
    let mut branches_resource =
        use_resource(move || async move { server_git_branches(site_id).await });
    let mut log_resource = use_resource(move || async move { server_git_log(site_id, 50).await });
        ...
        div { class: "glass-card rounded-2xl p-5 mb-5",
                ...
                div { class: "flex gap-2 flex-wrap",
                    button {
                        ...
                        onclick: move |_| {
                            ...
                            spawn(async move {
                                match server_git_generate_deploy_key(site_id).await {
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            ...
                            spawn(async move {
                                match server_detach_git_repo(site_id).await {
            ...
            if let Some(ref pub_key) = new_deploy_key() {
                div { class: "mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg",
                    ...
                    pre { class: "text-xs text-blue-900 font-mono break-all whitespace-pre-wrap select-all",
                        "{pub_key}"
                    }
                }
                ...
                div { class: "mt-4 p-3 bg-gray-50 border border-black/[0.08] rounded-xl",
                    p { class: "text-xs font-semibold text-gray-700 mb-1",
                        "Current deploy key (public):"
                    }
                    pre { class: "text-xs text-gray-700 font-mono break-all whitespace-pre-wrap select-all",
                        "{pub_key}"
                    }
                }
            }
        ...
        div { class: "glass-card rounded-2xl p-5 mb-5",
            div { class: "flex items-start gap-3 flex-wrap",
                button {
                    ...
                    onclick: move |_| {
                        ...
                        action_output.set(None);
                        spawn(async move {
                            match server_git_pull(site_id).await {
                                Ok(out) => {
                ...
                button {
                    ...
                    onclick: move |_| {
                        ...
                        action_output.set(None);
                        spawn(async move {
                            match server_git_push(site_id).await {
                                Ok(out) => {
                                    action_output.set(Some(out));
                                    log_resource.restart();
                                }
                ...
                div { class: "flex flex-1 gap-2 items-center min-w-[260px]",
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            ...
                            action_output.set(None);
                            spawn(async move {
                                match server_git_commit_and_push(site_id, msg).await {
                                    Ok(out) => {
                                        action_output.set(Some(out));
            ...
            if let Some(ref out) = action_output() {
                div { class: "mt-3 p-3 bg-gray-950 rounded-lg",
                    pre { class: "text-xs text-green-400 font-mono overflow-auto max-h-48 whitespace-pre-wrap",
                        "{out}"
                    }
                }
            }
            if let Some(ref err) = action_error() {
                div { class: "mt-3 p-3 bg-red-50 border border-red-200 rounded-lg",
                    pre { class: "text-xs text-red-700 font-mono overflow-auto max-h-32 whitespace-pre-wrap",
                        "{err}"
                    }
                }
            }
        ...
        div { class: "glass-card rounded-2xl overflow-hidden",
            div { class: "flex border-b border-gray-200",
                button {
                    ...
                    class: if active_tab() == "status" {
                        "border-rose-500 text-gray-700"
                    } else {
                    ...
                    match &*status_resource.read() {
                        Some(Ok(s)) => rsx! {
                            if s.trim().is_empty() {
                                ...
                                pre { class: "bg-gray-50 border border-black/[0.08] rounded-xl p-4 text-xs font-mono text-gray-800 overflow-auto max-h-64 whitespace-pre-wrap",
                                    "{s}"
                                }
                            }
                        },
                    ...
                    match &*branches_resource.read() {
                        Some(Ok(branch_list)) => rsx! {
                            if branch_list.is_empty() {
                                p { class: "text-[13px] text-gray-400",
                                    "No branches found. Pull from remote first."
                                }
                            } else {
                                div { class: "space-y-2",
        ...
        div { class: "p-6 lg:p-8",
            div { class: "mb-6",
                ...
                p { class: "text-[13px] text-gray-400 mt-1",
                    "Attach and manage Git repositories for your websites."
                }
            }
                ...
                match &*repo_resource.read() {
                    Some(Ok(None)) => rsx! {
                        GitNoRepo {
                            ...
                            on_attached: move |_| repo_resource.restart(),
                        }
                    },
    ...
    rsx! {
        div {
            ...
            div { class: "flex items-center gap-2",
                ...
                if is_current {
                    span { class: "px-1.5 py-0.5 text-xs bg-emerald-500 text-white rounded-full",
                        "current"
                    }
                }
            }
